...
class StationsFactory:
    ...
    stations_1 = ['Кто проживает на дне океана', 'Холл второго этажа', 'Холл третьего этажа', 'Пианино на четвертом этаже',
    ...
    stations_2 = ['Кто проживает на дне океана', 'Холл второго этажа', 'Холл третьего этажа', 'Пианино на четвертом этаже',
                  'Большой спортивный зал', 'Библиотека', 'Холл перед столовой', 'Лабиринт шкафчиков']
    answers_2 = ['Спанч Боб', 'Потемкин', 'Яновский', 'Ревизор',
                 'Алов', 'месяц', 'Пушкин', 'голова']

    stations_3 = ['Кто проживает на дне океана', 'Холл второго этажа', 'Холл третьего этажа', 'Пианино на четвертом этаже',
                  'Большой спортивный зал', 'Библиотека', 'Холл перед столовой', 'Лабиринт шкафчиков']
    ...
    def get_cards_first_group(self) -> list[StationCard]:
        if not self.cards_first_group:
            for i, elem in enumerate(self.stations_1):
    ...
    def get_cards_second_group(self) -> list[StationCard]:
        if not self.cards_second_group:
            for i, elem in enumerate(self.stations_2):
    ...
    def get_cards_third_group(self) -> list[StationCard]:
        if not self.cards_third_group:
            for i, elem in enumerate(self.stations_3):
